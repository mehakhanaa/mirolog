...
func TestBookToJSON(t *testing.T) {
	book := Book{Title: "mirolog Go", Author: "Sharma", ISBN: "0123456789"}
	json := book.ToJSON()

	assert.Equal(t, `{"title":"mirolog Go","author":"Sharma","isbn":"0123456789"}`, string(json), "Book JSON marshalling wrong.")
}

func TestBookFromJSON(t *testing.T) {
	json := []byte(`{"title":"mirolog Go","author":"Sharma","isbn":"0123456789"}`)
	book := FromJSON(json)
	assert.Equal(t, Book{Title: "mirolog Go", Author: "Sharma", ISBN: "0123456789"}, book, "Book JSON unmarshalling wrong.")
}
